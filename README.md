...
## 目录结构与关键模块
- 数据采样: [sampling.py](./sampling.py)
- 权重（无 STD，LDS 平滑；有 STD，高斯平滑）: [dataset_balance.py](./dataset_balance.py)
- 相关系数计算: [GMC.py](./GMC.py)
- 曲面拟合与绘图: [multi_surface.py](./multi_surface.py)
- STD 估计（无 STD 自动估计）: [std_deviation.py](./std_deviation.py)
- 主脚本（命令行入口）: [main.py](./main.py)
...
```bash
python main.py --pred-file ./test/pred_ssim_kadid10k.csv --weights-file ./results/kadid10k_Gauss/weights.txt --sample-file ./results/kadid10k_Gauss/sample100.xlsx --output ./test/ssim_kadid10k.html
```
